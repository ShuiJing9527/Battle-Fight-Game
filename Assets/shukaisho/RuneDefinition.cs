...
public class RuneDefinition
{
    ...
    {
        RuneDisplayText displayText = GetRuneDisplayText(type);
        return displayText != null ? BuildLocalizedFullDescription(displayText) : string.Empty;
    }

    private static string BuildLocalizedFullDescription(RuneDisplayText displayText)
    {
        GameLanguage language = GameLocalization.Instance != null
            ? GameLocalization.Instance.CurrentLanguage
            : GameLanguage.SimplifiedChinese;
        string runeHeader = language == GameLanguage.English
            ? "Rune Effects"
            : language == GameLanguage.Japanese ? "ルーン本体" : "符文本体效果";
        string setHeader = language == GameLanguage.English
            ? "Set Bonuses"
            : language == GameLanguage.Japanese ? "セット追加効果" : "套装额外效果";

        StringBuilder builder = new StringBuilder();
        builder.Append('【').Append(runeHeader).AppendLine("】");
        for (int i = 0; i < displayText.BaseEffects.Length; i++)
        {
            builder.Append(i + 1).Append(language == GameLanguage.English ? " Piece: " : language == GameLanguage.Japanese ? "個：" : "件：")
                .AppendLine(displayText.BaseEffects[i].GetCurrentLanguageText());
        }

        builder.Append('\n').Append('【').Append(setHeader).AppendLine("】");
        AppendLocalizedSetDescription(builder, displayText.Set2, language);
        AppendLocalizedSetDescription(builder, displayText.Set4, language);
        AppendLocalizedSetDescription(builder, displayText.Set5, language);
        return builder.ToString().Trim();
    }

    private static void AppendLocalizedSetDescription(StringBuilder builder, RuneSetDisplayText setText, GameLanguage language)
    {
        if (setText == null)
        {
            return;
        ...
        builder.Append(setText.RequiredCount)
            .Append(language == GameLanguage.English ? " Pieces - " : language == GameLanguage.Japanese ? "個・" : "件套·")
            .Append(setText.Name.GetCurrentLanguageText()).Append("：")
            .AppendLine(setText.Description.GetCurrentLanguageText());
    }
        ...
        switch (type)
        {
            case RuneType.Life:
                return new RuneDisplayText(
                    ...
                    new[]
                    {
                        L("Kills always drop 1 Life Soul and have a 20% chance to drop 1 extra Growth Soul.", "击杀敌人时，额外掉落1个生命之魂，并有20%概率额外掉落1个成长之魂。", "敵を倒すと生命ソウルを1個追加ドロップし、20%の確率で成長ソウルを1個追加ドロップ。"),
                        L("Each successful skill cast restores 2% max HP.", "每次施放技能时，恢复自身2%最大生命值。", "スキル発動時、最大HPの2%を回復。"),
            ...
            case RuneType.Shield:
                return new RuneDisplayText(
                    ...
                    new[]
                    {
                        L("After 3s without monster damage, refill shield up to 30% max HP.", "连续3秒未受到怪物伤害后，自动恢复护盾，直到达到30%最大生命值。", "3秒間モンスターダメージを受けないと、最大HP30%までシールド回復。"),
                        L("While shielded, damage dealt +30%.", "自身拥有护盾时，造成的伤害提高30%。", "シールド中、与ダメージ+30%。"),
                        L("Kills drop 1 extra Function Soul.", "击杀敌人时，额外掉落1个功能之魂。", "敵を倒すと機能ソウルを1個追加ドロップ。"),
                    ...
                    S(2, "Solid Barrier", "坚固屏障", "堅固な結界", "While shielded, shield damage taken -5%. This only affects the shield portion.", "自身拥有护盾时，护盾受到的伤害降低5%，只影响护盾承受的部分。", "シールドが受けるダメージ-5%。シールド部分のみ。"),
                    ...
                    S(5, "Unfallen Fortress", "不落要塞", "不落の要塞", "The unified shield cap remains 150% max HP. Shield Rune bonuses improve shield gain and shielded benefits but cannot exceed this cap. Elite/Boss kills grant permanent Shield Efficiency +10%, up to 300%.", "当前版本统一护盾上限为150%最大生命值。护盾符文提高护盾获取和持盾收益，但不能突破该上限。击杀精英或Boss时，护盾效率永久+10%，最高300%。", "現バージョンの共通シールド上限は最大HP150%。シールド獲得量とシールド中の恩恵は強化されるが、この上限は超えない。エリート/Boss撃破でシールド効率+10%（最大300%）。"));
            ...
            case RuneType.Mana:
                return new RuneDisplayText(
                    ...
                    new[]
                    {
                        ...
                        L("Kills drop 1 extra Energy Soul. Mana recovery overflow becomes Mana Overflow, capped at 200% max mana.", "击杀敌人时，额外掉落1个能量之魂。获得魔力时，超过最大魔力值的部分转化为Mana Overflow，最多至200%最大魔力值。", "敵を倒すとエネルギーソウルを1個追加ドロップ。過剰MP回復はMana Overflowになり、最大MP200%まで蓄積。"),
                    ...
                    S(2, "Mana Flow", "魔力回流", "マナ還流", "After a successful cast, refund 8% of the skill's base mana cost.", "成功施放技能后，返还该技能基础魔力消耗的8%。", "スキル成功後、基本MP消費の8%を返還。"),
                    S(4, "Arcane Resonance", "奥术共鸣", "奥術共鳴", "When Mana Rune extra spending totals 20% max mana, gain 8s of +25% damage, +15% cooldown recovery speed, and +75% Mana Rune bonus contribution. The counter resets on trigger.", "通过魔力符文额外耗蓝累计达到20%最大魔力值时，获得8秒奥术共鸣：造成伤害+25%，技能冷却恢复速度+15%，魔力符文额外耗蓝产生的技能强化效果+75%。触发后累计值清零。", "追加MP消費が最大MP20%に達すると8秒間、与ダメージ+25%、CD回復速度+15%、マナルーン強化貢献+75%。発動後カウントはリセット。"),
                    S(5, "Arcane Overload", "奥术超载", "奥術過負荷", "Energy Soul mana recovery becomes 400%, Mana Overflow cap becomes 300% max mana, and Elite/Boss kills grant permanent Mana Conversion Efficiency +10%, up to 300%.", "能量之魂的魔力恢复量提高至400%，Mana Overflow上限提高至300%最大魔力值。击杀精英或Boss时，魔力转化效率永久+10%，最高300%。", "エネルギーソウルのMP回復量400%、Mana Overflow上限最大MP300%。エリート/Boss撃破でマナ変換効率+10%（最大300%）。"));
            ...
            case RuneType.Thorn:
                return new RuneDisplayText(
                    ...
                    new[]
                    {
                        ...
                        L("When hit by a monster, release a ranged Thorn Counter explosion around you.", "受到怪物攻击时，自动在自身周围释放范围荆棘反击爆炸。", "モンスターに攻撃されると、自身の周囲に範囲型Thorn Counter爆発を自動発動。")
                    },
                    ...
                    S(4, "Thorn Backlash", "荆棘反噬", "棘の反噬", "When taking monster damage, this hit is reduced by an extra 30% and only that attacker takes Thorn damage equal to 200% current Thorn value. This is not an area explosion. 5s cooldown.", "受到怪物伤害时，本次伤害额外降低30%，并只对该攻击者造成200%当前荆棘值的单体荆棘伤害，不产生范围爆炸。5秒冷却。", "モンスターダメージをさらに30%軽減し、その攻撃者だけに現在棘値200%の単体棘ダメージ。範囲爆発は発生しない。5秒CD。"),
                    S(5, "Thousand-Thorn Counter", "万刺反击", "千の棘", "Unlock the ranged Thorn Counter explosion around you, increase Thorn damage by 100%, and reduce its cooldown to 2s. Elite/Boss kills grant permanent Thorn Efficiency +10%, up to 300%.", "解锁自身周围的范围荆棘反击爆炸，荆棘伤害额外提高100%，范围反击冷却缩短至2秒。击杀精英或Boss时，荆棘效率永久+10%，最高300%。", "自身周囲の範囲型Thorn Counter爆発を解禁し、棘ダメージ+100%、範囲反撃CD2秒。エリート/Boss撃破で棘効率+10%（最大300%）。"));
            ...
            case RuneType.Luck:
                return new RuneDisplayText(
                    ...
                    {
                        L("Luck +5.", "幸运+5。", "運+5。"),
                        L("When a Growth Soul is generated, 30% chance to increase its point by 1, up to 8.", "成长之魂生成时，有30%概率使其点数+1，最高8点。", "成長ソウル生成時、30%の確率で値+1（最大8）。"),
                        L("Kills have a 15% chance to drop 1 extra Growth Soul.", "击杀敌人时，有15%概率额外掉落1个成长之魂。", "敵を倒すと15%の確率で成長ソウルを1個追加ドロップ。"),
                        L("Picking up any Soul has a 12% chance to copy 1 Soul of the same type. The copy is fixed at 2 points.", "拾取任意灵魂时，有12%概率额外复制1个同类灵魂，复制灵魂固定为2点。", "ソウル取得時、12%の確率で同種ソウルを1個コピー。値は2固定。"),
                        L("Elite/Boss kills drop 2 extra Growth Souls.", "击杀精英敌人或Boss时，额外掉落2个成长之魂。", "エリート/Boss撃破時、成長ソウルを2個追加ドロップ。")
                    },
    ...
    public static RuneDefinition CreateDefaultRune(RuneType type)
    {
        ...
        switch (type)
        {
            case RuneType.Life:
                ...
                rune.tier1Effect = "击杀敌人时，额外掉落1个生命之魂，并有20%概率额外掉落1个成长之魂。";
                rune.tier2Effect = "每次成功施放技能时，恢复自身2%最大生命值。";
            ...
            case RuneType.Shield:
                ...
                rune.tier1Effect = "连续3秒未受到怪物伤害后，自动恢复护盾，直到达到30%最大生命值。";
                rune.tier2Effect = "自身拥有护盾时，造成的伤害提高30%。";
                rune.tier3Effect = "击杀敌人时，额外掉落1个功能之魂。";
                rune.tier4Effect = "自身获得的护盾量提高100%，受护盾效率影响。";
                rune.tier5Effect = "每次成功施放技能后，下一次对敌人造成伤害时，追加相当于自身当前护盾值15%的伤害，受护盾效率影响。";
                rune.setBonusEffect = "2件套·坚固屏障：自身拥有护盾时，护盾受到的伤害降低5%，只影响护盾承受的部分。\n4件套·壁垒重构：护盾被怪物击破时，获得3秒壁垒重构：受到怪物伤害降低40%，免疫击退和硬直。结束时获得30%最大生命值护盾。15秒冷却。\n5件套·不落要塞：当前版本统一护盾上限为150%最大生命值。护盾符文提高护盾获取和持盾收益，但不能突破该上限。击杀精英或Boss时，护盾效率永久+10%，最高300%。";
                break;
            ...
            case RuneType.Mana:
                ...
                rune.tier3Effect = "击杀敌人时，额外掉落1个能量之魂。获得魔力时，超过最大魔力值的部分转化为魔力溢出，最多至200%最大魔力值。";
                ...
                rune.setBonusEffect = "2件套·魔力回流：成功施放技能后，返还该技能基础魔力消耗的8%。\n4件套·奥术共鸣：通过魔力符文额外耗蓝累计达到20%最大魔力值时，获得8秒奥术共鸣：造成伤害+25%，技能冷却恢复速度+15%，魔力符文额外耗蓝产生的技能强化效果+75%。触发后累计值清零。\n5件套·奥术超载：能量之魂的魔力恢复量提高至400%，魔力溢出上限提高至300%最大魔力值。击杀精英或Boss时，魔力转化效率永久+10%，最高300%。";
                break;
            ...
            case RuneType.Thorn:
                ...
                rune.tier5Effect = "受到怪物攻击时，自动在自身周围释放范围荆棘反击爆炸。";
                rune.setBonusEffect = "2件套·倒刺汲取：荆棘伤害成功命中后恢复2%最大生命值，每1秒最多1次。\n4件套·荆棘反噬：受到怪物伤害时，本次伤害额外降低30%，并只对该攻击者造成200%当前荆棘值的单体荆棘伤害，不产生范围爆炸。5秒冷却。\n5件套·万刺反击：解锁自身周围的范围荆棘反击爆炸，荆棘伤害额外提高100%，范围反击冷却缩短至2秒。击杀精英或Boss时，荆棘效率永久+10%，最高300%。";
                break;
            ...
            case RuneType.Luck:
                ...
                rune.tier2Effect = "成长之魂生成时，有30%概率使其点数+1，最高8点。";
                rune.tier3Effect = "击杀敌人时，有15%概率额外掉落1个成长之魂。";
                rune.tier4Effect = "拾取任意灵魂时，有12%概率额外复制1个同类灵魂，复制灵魂固定为2点。";
                rune.tier5Effect = "击杀精英敌人或Boss时，额外掉落2个成长之魂。";
